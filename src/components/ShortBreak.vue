...
const DEFAULT_MINUTES = Number(
  localStorage.getItem("shortBreaktimer") ?? INITIAL_MINUTES
);
...
function restart() {
  ...
  localStorage.setItem("shortBreaktimer", INITIAL_MINUTES);
  stop();
}
...
watch(input, (newInputValue) => {
  ...
  localStorage.setItem("shortBreaktimer", newInputValue);
});
